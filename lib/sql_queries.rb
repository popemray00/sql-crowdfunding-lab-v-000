...
"SELECT projects.title SUM(pledges.amount) - projects.funding_goal FROM projects JOIN pledges ON projects.id = pledges.projects.id WHERE projects.funding_goal >= projects.funding_goal GROUP BY projects.title HAVING SUM(pledges.amount - projects.funding_goal >= 0) "
end
